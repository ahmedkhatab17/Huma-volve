...
class _SeafoodScreenState extends State<SeafoodScreen> {
  ...
  Future<void> _loadMeals(String category) async {
    ...
    final result = await _apiService.getMealsByCategory(category);
    if (!mounted) return;

    result.fold(
      (failure) => setState(() {
        _mealsError = failure.errorMessage;
        _mealsLoading = false;
      }),
      (meals) => setState(() {
        _meals = meals;
            ...
            child: Column(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  ...
                  onPressed: () => _loadMeals(_selectedCategory!),
